...
public class TableFixHeaders extends ViewGroup {
	...
	private int scrollX;
	private int scrollY;
	private int firstRow;	//row of cell in angle 
	private int firstColumn;//col of cell in angle 
	private int[] widths;
	private int[] heights;
	...
	public void setAdapter(TableAdapter adapter) {
		...
		adapter.setTableFixHeaders(this);
	...
	public void printAllData() {
		Log.d("AndroidSFR", "rowViewList" );
		for (int i = 0; i < rowViewList.size(); i++) {
			Log.d("AndroidSFR", ((TextView)rowViewList.get(i)).getText().toString());
		}
		Log.d("AndroidSFR", "colViewList" );
		for (int i = 0; i < columnViewList.size(); i++) {
			Log.d("AndroidSFR", ((TextView)columnViewList.get(i)).getText().toString());
		}
		Log.d("AndroidSFR", "dataViewList" );
		for (int i = 0; i < bodyViewTable.size(); i++) {
			for(int j = 0; j < bodyViewTable.get(0).size(); j++) {
				Log.d("AndroidSFR", ((TextView)bodyViewTable.get(i).get(j)).getText().toString());
			}
		}
	}
	...
	public int[] getViewCoordinates(View view) {
		...
		//assume that view.getTranslationX() == view.getTranslationY() == 0
		int viewX = view.getLeft();// + (int)view.getTranslationX();
		int viewY = view.getTop();// + (int)view.getTranslationY();
		
		int col = firstColumn;
		int row = firstRow;
		int left = widths[0] - scrollX;
		for (col = firstColumn; col < columnCount && left < width; col++) {
			if (left >= viewX) {
				break;
			}
			left += widths[col + 1];
		...
		if (viewY == 0) {
			...
			for (row = firstRow; row < rowCount && top < height; row++) {
				if (top >= viewY) {
					break;
				}
				top += heights[row + 1];
		...
		row += 1; //header is 0, not -1;
		Log.d("AndroidSFR", String.format("%d row, %d col", row, col));
	...
	public void setNeedRelayout(boolean needRelayout) {
